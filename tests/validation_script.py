# validation_script_minimal_queries.py
# Purpose: Answer 40 analytical questions using Aqueduct 4.0 database
...
# ---------- Build baseline/future/continent views ----------
conn.execute("""
...
# optional continent CSV
has_cc = False
if os.path.exists(CC_CSV):
    conn.execute(f"""
    CREATE OR REPLACE TEMP VIEW country_continent AS
    SELECT TRIM(country) AS country, TRIM(continent) AS continent
...
if has_cc:
    show("Average groundwater depletion across Africa",
         """
         SELECT ROUND(AVG(b.gtd_raw),4) AS avg_gtd
         FROM v_country_baseline b JOIN country_continent cc USING(country)
...
if has_cc:
    show("Mean flood risk across Europe",
         """
         SELECT ROUND(AVG(b.rfr_score),4) AS mean_flood_risk
         FROM v_country_baseline b JOIN country_continent cc USING(country)
         WHERE cc.continent='Europe';
         """)

if has_cc:
    show("Average drought frequency in Middle Eastern countries",
         """
         SELECT ROUND(AVG(b.drr_score),4) AS avg_drought
         FROM v_country_baseline b
         WHERE b.country IN ('Saudi Arabia','Iran','Iraq','UAE','Jordan','Oman','Israel','Lebanon','Qatar');
         """)
...
def compare(qtitle, metric, c1, c2, table="v_country_baseline"):
    show(qtitle, f"SELECT country, ROUND({metric},4) AS value FROM {table} WHERE country IN ('{c1}','{c2}') ORDER BY value DESC;")
