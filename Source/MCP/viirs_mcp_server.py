...
def safe_stderr_print(msg):
    """Safely print to stderr, catching BrokenPipeError for HTTP adapter compatibility"""
    try:
        print(msg, file=sys.stderr)
        sys.stderr.flush()
    except (BrokenPipeError, OSError):
        pass  # Ignore broken pipe errors when running via HTTP adapter

safe_stderr_print("Starting VIIRS MCP Server...")

# Initialize MCP server
app = Server("viirs-fire-server")

# Database path - read from environment variable or use default filename
DB_FILENAME = os.getenv("VIIRS_DUCKDB_PATH", "VIIRS_Thermal_Database.duckdb")
SCRIPT_DIR = Path(__file__).resolve().parent

# Keep a default path value (may be relative) for logging; actual resolution happens at connect time
...
def get_db_connection():
    ...
    # 1) If env var supplied an absolute/relative path, try it first.
    # If the env var points to a directory, append the DB filename.
    ...
    # 2) Prefer a test copy placed next to this script (Source/MCP folder).
    candidates.append(SCRIPT_DIR / DB_FILENAME)

    # 3) A canonical server location (common in our environment) as a fallback.
    candidates.append(Path("/srv/viirs_database") / DB_FILENAME)

    # 4) If DB_FILENAME is an absolute path or the repo root contains it, try them.
    candidates.append(Path(DB_FILENAME))
    candidates.append(SCRIPT_DIR.parent / DB_FILENAME)

    # 5) Try current working directory as a last local fallback
    candidates.append(Path.cwd() / DB_FILENAME)

    # Normalize and deduplicate candidates while preserving order
    seen = set()
    cleaned = []
    for p in candidates:
        try:
    ...
    safe_stderr_print("DB connection candidates (in order):")
    for p in candidates:
        safe_stderr_print(f"  - {p} -> exists: {p.exists()}")

    # Pick first existing candidate
    chosen = next((p for p in candidates if p.exists()), None)
    if chosen is None:
        # Helpful error listing attempted paths
        attempted = "\n".join(f"  - {p}" for p in candidates)
        msg = (
            f"Database not found. Tried these locations:\n{attempted}\n\n"
            "Set VIIRS_DUCKDB_PATH env var to the DB absolute path or place the DB file "
            "next to this script or in the current working directory."
        )
        safe_stderr_print(f"ERROR: {msg}")
        raise FileNotFoundError(msg)

    # Update DB_PATH for logs and subsequent calls
    global DB_PATH
    DB_PATH = chosen.resolve()
    safe_stderr_print(f"Connecting to DuckDB at: {DB_PATH}")
...
async def run_count_viirs_by_place(params: dict[str, Any]) -> TextContent:
    ...
    # Use COUNT(*) to return a single integer (ORIGINAL - NOT CHANGED)
    query = f"""
        SELECT COUNT(*) as cnt
        FROM VIIRS_Thermal_Records
        WHERE latitude BETWEEN {min_lat} AND {max_lat}
...
async def call_tool(name: str, arguments: Any) -> list[TextContent]:
    ...
    conn = None
    try:
        safe_stderr_print(f"DEBUG: call_tool invoked: {name}")
        conn = get_db_connection()
        ...
        safe_stderr_print(f"ERROR in call_tool('{name}'): {e}")
        import traceback
        try:
        ...
        try:
            ...
            safe_stderr_print("WARNING: failed to close DB connection")

async def main():
    """Run the MCP server"""
    try:
        safe_stderr_print("Initializing MCP server stdio...")

        async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
            safe_stderr_print("MCP server streams created, starting server...")

            try:
                init_opts = app.create_initialization_options()
            except Exception as e:
                init_opts = None
                safe_stderr_print(f"DEBUG: Failed to create initialization options: {e}")
            safe_stderr_print(f"DEBUG: initialization options: {init_opts}")

            safe_stderr_print("DEBUG: calling app.run()")
            try:
                await app.run(read_stream, write_stream, init_opts)
                safe_stderr_print("DEBUG: app.run() returned normally")
            except Exception as e:
                safe_stderr_print(f"ERROR: app.run() raised an exception: {e}")
                import traceback
                try:
                    traceback.print_exc(file=sys.stderr)
                except (BrokenPipeError, OSError):
        ...
        safe_stderr_print(f"ERROR in main: {e}")
        import traceback
        try:
            traceback.print_exc(file=sys.stderr)
        except (BrokenPipeError, OSError):
...
if __name__ == "__main__":
    try:
        safe_stderr_print("DEBUG: Attempting to run app.run_stdio_async()")
        asyncio.run(app.run_stdio_async())
    except AttributeError:
        try:
            safe_stderr_print("DEBUG: run_stdio_async not available, falling back to main()")
            asyncio.run(main())
        except Exception as e:
            safe_stderr_print(f"FATAL ERROR in fallback run: {e}")
            import traceback
            try:
                traceback.print_exc(file=sys.stderr)
            except (BrokenPipeError, OSError):
        ...
        safe_stderr_print(f"FATAL ERROR: {e}")
        import traceback
        try:
            traceback.print_exc(file=sys.stderr)
        except (BrokenPipeError, OSError):
